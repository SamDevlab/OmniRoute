...
test("authoritative gate requires both target identities, complete artifacts, and independent accounting", () => {
  const source = fivePairGateSource();
...
test("Governor execution identity fails closed unless target and required connection are both proven", () => {
  const start = harnessSource.indexOf("const targetIdentity =", harnessSource.indexOf("runGovernorE2E"));
...
test("target revalidation evaluates all matching connection-specific targets instead of first-match state", () => {
  ...
  assert.match(source, /candidates\.some\(\(candidate\) => candidate\.quotaCutoffBlocked !== true\)/);
