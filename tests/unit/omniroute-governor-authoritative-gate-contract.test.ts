...
test(
  "authoritative gate still rejects structural, identity, and runtime failures",
  () => {
    const source = fivePairGateSource();
...
test(
  "authoritative conclusion refuses latency claims when no pair is latency-comparable",
  () => {
    const source = functionSource(
      "authoritativeConclusion",
      "calibrationRecoverySummary"
    );
    ...
    assert.match(
      source,
      /latencyComparablePairs\.length === 0\) return "E2E_INCONCLUSIVE"/
    );
